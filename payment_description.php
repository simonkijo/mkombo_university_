...
<div class="box box-primary" style="padding-left:5%; padding-right:5%; padding-bottom:3%" >
 ...
 <h3 class="box-title" style="margin-left:40%;">fees Diploma</h3>
 </div>
   <table id="example1" class="table table-bordered table-striped" style="">
      <thead><tr ><th>Description</th><th colspan="3">Diploma Programmes</th></tr></thead>
		 <tbody>
		 <tr><td> </td><td>Year 1</td><td>Year 2</td><td>Year 3</td></tr>	
				<?php 
             	//$conn=mysql_connect("localhost","root","") or die("not connected");
	            //$select_db=mysql_select_db('mkombo_university',$conn);
	            $result=mysql_query("SELECT * FROM diplom_payment") or die(mysql_error());//card_id="$_POST['password']" AND your_full_name="$_POST['username']");
			  ...
			  $add=mysql_query('SELECT SUM(year_1),SUM(year_2),SUM(year_3) from diplom_payment');
            ...
            <tr><td style="width:30%; padding-left:10%;"><label>Full Name</label></td><td><?php echo $fname.' '.$mname.' '.$sname; ?></td></tr>
				<tr><td style="width:30%; padding-left:10%;"><label>Level</label></td><td><?php echo $level; ?></td></tr>
			      <tr><td style="width:30%; padding-left:10%;"><label>Course</label></td><td><?php echo $course; ?></td></tr>
					 ...
					 <tr><td style="width:30%; padding-left:10%;"><label>Eceed</label></td><td><?php echo $eceed; ?></td></tr>
					<tr><td colspan="2%" align="center"><font size="1%"> from the financial department</font></td></tr>

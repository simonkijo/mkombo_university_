<?php 
	if(isset($_POST['allow_all'])){
		$query = "SELECT `module_title` FROM `mkombo_university`.`modules` WHERE `programme`='Ordinary Diploma'";
		...
		for($i=0;$i<count($module_title);$i++){
			$q = "UPDATE `mkombo_university`.`modules` SET `access`='YES' WHERE `programme`='Ordinary Diploma'";
			$r = mysql_query($q);
		}
		if($r){
			$success[0] = 'allow_all';

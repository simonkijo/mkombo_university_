<?php 
	...
	if(isset($_POST['username']) && isset($_POST['email'])){
		...
		if(!empty($username) && !empty($email)){
			$query = "SELECT `username`,`email_address` FROM `mkombo_university`.`admin` WHERE `username`='".mysql_real_escape_string($username)."' AND `email_address`='".mysql_real_escape_string($email)."'";
			...
			if($num_row == 0){
				...
				changePasswordStaff($usern,$email_ad,'admin');
			}
		}
	}
?>
